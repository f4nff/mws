...
//	l.srv = &http.Server{Addr: addr, Handler: mux}
	l.srv = &http.Server{Addr: addr,
		Handler: mux,
		ReadTimeout:  time.Duration( 65* time.Second ),   //keep alive timeout  65s
	}	
...
//	l.srv = &http.Server{Addr: addr, Handler: mux}
	l.srv = &http.Server{Addr: addr,
		Handler: mux,
		ReadTimeout:  time.Duration( 65* time.Second ),   //mws keepalive timeout  65s
	}
	ln, err := net.ListenTCP("tcp", tcpAddr)
	if err != nil {
		return nil, err
...
func WSSListener(addr string, tlsConfig *tls.Config, options *WSOptions) (Listener, error) {
	...
	l.srv = &http.Server{
		...
		TLSConfig: tlsConfig,
		Handler:   mux,
		ReadTimeout:  time.Duration( 65* time.Second ),  //wss keepalive timeout  65s
	}
...
func MWSSListener(addr string, tlsConfig *tls.Config, options *WSOptions) (Listener, error) {
	...
	l.srv = &http.Server{
		...
		TLSConfig: tlsConfig,
		Handler:   mux,
		ReadTimeout:  time.Duration( 65* time.Second ),  //mwss keepalive timeout  65s
	}
